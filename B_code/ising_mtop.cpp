//This programme again use the normal ising model as the backbone and insert a B fiend, we vary B from positive to negative
//The B field can change from negative to positive by changing the for loop
#include <iostream>
#include <iomanip>
#include <vector>
#include <fstream>
...
int main(){
	...
	ofstream ans("mtop_1.0_data.txt");
...
for (double uB=-1.0; uB<=1.0; uB=uB+0.01) {//for loop that loop the uB from -1 to 1
	//clean the variables
	E=0.0;
	S=0.0;
        ...
        //iniitiase energy and spin
        for(int i=0; i<n; i++){
        for(int j=0; j<n;j++){
		E=E-2.0*vec[i][j]*(vec[mod((i-1),n)][j]+vec[(i+1)%n][j]+vec[i][mod((j-1),n)]+vec[i][(j+1)%n]);
		S=S+vec[i][j];
                }}
	E=E-uB*S;
	...
	gsl_rng_set(r, k); //set the gsl random number generator and different seedx
	k=0;
	...
	//Plz see isingN100.cpp for more information the below algo
